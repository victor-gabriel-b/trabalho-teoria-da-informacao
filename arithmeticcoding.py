...
class ArithmeticEncoder(ArithmeticCoderBase):
	...
	def write(self, freqs, symbol, exclude=[]):
		#freqs = deepcopy(freqs)

		# Remove the frequencies that are now impossible (the exclude list)
		saved_symbols = []
		if len(exclude)>0:
			for excluded_symbol in exclude:
				saved_symbols.append(freqs.get(excluded_symbol))
				freqs.set(excluded_symbol, 0)

		# Remove escape character, if applicable
		freqs.try_remove_escape(256) # NOVA MUDANCA
		...
		for i in range(len(saved_symbols)):
			old_freqs.set(exclude[i], saved_symbols[i])
...
class ArithmeticDecoder(ArithmeticCoderBase):
	...
	def read(self, freqs, exclude=[]):
		...
		srange = self.high - self.low + 1
		offset = self.code - self.low
		value = ((offset + 1) * total - 1) // srange
		assert value * srange // total <= offset
		...
		symbol = start
		assert freqs.get_low(symbol) * srange // total <= offset < freqs.get_high(symbol) * srange // total
		self.update(freqs, symbol)
...
class FlatFrequencyTable(FrequencyTable):
	...
	def try_remove_escape(self, escapesymbol):
		if self.get(escapesymbol) != 0:
			self.numsymbols -= 1
...
class SimpleFrequencyTable(FrequencyTable):
	...
	# Returns a list of all symbols with non-zero frequency.
	def list_non_zero(self):
		return self.non_zero
	
	# Removes the escape character if all symbols have been seen. Otherwise, does nothing.
	def try_remove_escape(self, escapesymbol):
		assert len(self.frequencies) == 257
		if self.frequencies[256] == 0:  # Can't remove if it's already gone
			return
		if self.list_non_zero()[escapesymbol] == len(self.frequencies):
			self.set(escapesymbol, 0)
